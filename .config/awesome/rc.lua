...
local globalKeys = myTable.join(
	...
	awful.key({ modkey }, "d", function()
		menubar.show()
	end, { description = "Show awesome menubar.", group = "Hotkeys" }),
	awful.key({ modkey }, "w", function()
		awful.util.rcMainMenu:show()
	end, { description = "Show the main menu.", group = "Hotkeys" }),

	-- Awesome
	awful.key({ modkey }, "F1", hotkeys_popup.show_help, { description = "Launches this help.", group = "Awesome" }),
	awful.key({ modkey }, "s", hotkeys_popup.show_help, { description = "Launches this help.", group = "Awesome" }),
	awful.key({ modkey }, "r", awesome.restart, { description = "Reloads awesome.", group = "Awesome" }),
	...
	end, { description = "Toggle top wibox.", group = "Awesome" }),

	-- Super + ... function (F'x') keys.
	awful.key({ modkey }, "F2", function()
		awful.util.spawn(guiEditor)
	end, { description = "Launch the text editor.", group = "Function keys" }),
	awful.key({ modkey }, "F3", function()
		awful.util.spawn(filemanager)
	end, { description = "Launch the filemanager", group = "Function keys" }),
	awful.key({ modkey }, "F4", function()
		awful.util.spawn("xfc-terminal --drop-down")
	end, { description = "Dropdown terminal.", group = "Function keys" }),

	-- Super + ...
	awful.key({ modkey }, "Return", function()
		awful.util.spawn(terminal)
	end, { description = "Launch the terminal.", group = "Hotkeys" }),
	awful.key({ modkey, "Shift" }, "Return", function()
		awful.util.spawn(filemanager)
	end, { description = "Launches the filemanager.", group = "Hotkeys" }),
	awful.key({ modkey }, "b", function()
		awful.util.spawn(browser)
	end, { description = "Launch default browser.", group = "Hotkeys" }),
	awful.key({ modkey }, "e", function()
		awful.util.spawn(guiEditor)
	end, { description = "Launch graphical text editor.", group = "Hotkeys" }),
	awful.key({ modkey }, "t", function()
		awful.util.spawn(editor)
	end, { description = "Launch default terminal editor.", group = "Hotkeys" }),
	awful.key({ modkey }, "l", function()
		awful.util.spawn(lockscreen)
	end, { description = "Locks the screen on demand.", group = "Hotkeys" }),
	awful.key({ modkey }, "c", function()
		awful.util.spawn("code")
	end, { description = "Launches Visual Studio Code.", group = "Hotkeys" }),
	...
	awful.key({ modkey, ctrl }, "v", function()
		awful.spawn.with_shell("xsel -b | xsel")
	end, { description = "Copy gtk to terminal.", group = "Hotkeys" }),

	-- Super + ... eos apps.
	awful.key({ modkey, "Shift" }, "w", function()
		awful.util.spawn("eos-welcome --enable")
	end, { description = "EndeavourOS welcome app.", group = "EOS Apps" }),
	awful.key({ ctrl, altkey }, "l", function()
		awful.util.spawn("eos-log-tool")
	end, { description = "EndeavourOS log tool.", group = "EOS Apps" }),
	awful.key({ modkey, "Shift" }, "i", function()
		awful.util.spawn("eos-apps-info")
	end, { description = "EndeavourOS log tool.", group = "EOS Apps" }),
	awful.key({ modkey, "Shift" }, "r", function()
		awful.util.spawn("reflector-simple")
	end, { description = "EndeavourOS reflector simple.", group = "EOS Apps" }),
	awful.key({ modkey, "Shift" }, "m", function()
		awful.util.spawn(terminal .. "-e eos-rankmirrors")
	end, { description = "EndeavourOS rank mirrors.", group = "EOS Apps" }),

	-- Screenshots
	awful.key({}, "Print", function()
		awful.util.spawn("xfce4-screenshooter -i")
	end, { description = "Use xfce screenshooter.", group = "Screenshots" }),
	...
	awful.key({ altkey, ctrl, "Shift" }, "Tab", awful.tag.viewnext, { description = "View next.", group = "Tag" }),
	...
	awful.key({ ctrl, modkey }, "Down", function()
		awful.client.focus.global_bydirection("down")
		if client.focus then
			client.focus:raise()
		end
	end, { description = "Focus down.", group = "Client" }),
	awful.key({ ctrl, modkey }, "Up", function()
		awful.client.focus.global_bydirection("up")
		if client.focus then
			client.focus:raise()
		end
	end, { description = "Focus up.", group = "Client" }),
	awful.key({ ctrl, modkey }, "Left", function()
		awful.client.focus.global_bydirection("left")
		if client.focus then
			client.focus:raise()
		end
	end, { description = "Focus left.", group = "Client" }),
	awful.key({ ctrl, modkey }, "Right", function()
		awful.client.focus.global_bydirection("right")
		if client.focus then
			client.focus:raise()
		end
	...
	awful.key({ altkey, ctrl }, "j", function()
		lain.util.useless_gaps_resize(1)
	end, { description = "Increment useless gaps.", group = "Tag" }),
	awful.key({ altkey, ctrl }, "h", function()
		lain.util.useless_gaps_resize(-1)
	...
	end, { description = "Volumn up.", group = "Audio" }),
	awful.key({ modkey, "Shift" }, "Down", function()
		os.execute(string.format("amixer -q set %s 1%%-", beautiful.volume.channel))
		beautiful.volume.update()
	end, { description = "Volumn down.", group = "Audio" }),
	awful.key({ modkey, altkey }, "m", function()
		os.execute(string.format("amixer -q set %s toggle", beautiful.volume.togglechannel or beautiful.volume.channel))
		beautiful.volume.update()
	end, { description = "Volumn mute.", group = "Audio" }),
	awful.key({ modkey, "Shift" }, "m", function()
		os.execute(string.format("amixer -q set %s 100%%", beautiful.volume.channel))
		beautiful.volume.update()
	end, { description = "Volumn full.", group = "Audio" }),
	awful.key({ modkey, "Shift" }, "0", function()
		os.execute(string.format("amixer -q set %s 0%%", beautiful.volume.channel))
		beautiful.volume.update()
	end, { description = "Volumn 0.", group = "Audio" })
...
awful.rules.rules = {
	...
	-- find class or role via xprop command
	{ rule = { class = guiEditor }, properties = { maximized = true } },

	-- Set applications to be maximized at startup with floating disabled.
	-- find class or role via xprop command
	{ rule = { class = filemanager }, properties = { maximized = true, floating = false } },
	{ rule = { class = "code" }, properties = { maximized = false, floating = false } },
	{ rule = { class = "Arandr" }, properties = { maximized = false, floating = false } },
